...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "check_membership":
        ...
        return await handle_send_report(update, context, user.id)
...
async def handle_phone_input_state(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle phone number input in WAITING_PHONE state"""
    user = update.effective_user
    text = update.message.text
    
    logger.info(f"Received phone input from {user.id}: {text}")
    ...
    try:
        ...
        await msg.edit_text(
            f"❌ <b>Error sending code:</b> {str(e)}\n\nPlease try again with /start",
            parse_mode=ParseMode.HTML
        )
        context.user_data.clear()
        return ConversationHandler.END
...
async def handle_code_input_state(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle OTP code input in WAITING_CODE state"""
    user = update.effective_user
    text = update.message.text
    phone = context.user_data.get("phone")
    phone_code_hash = context.user_data.get("phone_code_hash")
    
    logger.info(f"Received code input from {user.id}")
...
async def handle_password_input_state(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle 2FA password input in WAITING_PASSWORD state"""
    user = update.effective_user
    text = update.message.text
    phone = context.user_data.get("phone")
    phone_code_hash = context.user_data.get("phone_code_hash")
    
    logger.info(f"Received password input from {user.id}")
...
async def handle_group_link_state(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle group link input in WAITING_GROUP_LINK state"""
    text = update.message.text
    logger.info(f"Received group link: {text}")
...
async def handle_target_link_state(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle target link input in WAITING_TARGET_LINK state"""
    text = update.message.text
    logger.info(f"Received target link: {text}")
...
async def handle_report_count_state(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle report count input in WAITING_REPORT_COUNT state"""
    text = update.message.text
    logger.info(f"Received report count: {text}")
...
async def handle_description_state(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle description input and start reporting in WAITING_DESCRIPTION state"""
    user = update.effective_user
    text = update.message.text
    description = "" if text.lower() == "skip" else text
    
    logger.info(f"Received description from {user.id}, starting report")
...
"""
        ...
        await status_msg.edit_text(
            f"❌ <b>Error during reporting:</b> {str(e)}",
            parse_mode=ParseMode.HTML
        )
...
async def handle_broadcast_state(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle broadcast message in WAITING_BROADCAST state"""
    user = update.effective_user
...
async def handle_sudo_id_add_state(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle add sudo user ID input"""
    user = update.effective_user
    text = update.message.text
...
async def handle_sudo_id_remove_state(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle remove sudo user ID input"""
    text = update.message.text
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            WAITING_PHONE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, handle_phone_input_state)
            ],
            WAITING_CODE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, handle_code_input_state)
            ],
            WAITING_PASSWORD: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, handle_password_input_state)
            ],
            WAITING_GROUP_LINK: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, handle_group_link_state)
            ],
            WAITING_TARGET_LINK: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, handle_target_link_state)
            ],
            WAITING_REPORT_COUNT: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, handle_report_count_state)
            ],
            WAITING_DESCRIPTION: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, handle_description_state)
            ],
            WAITING_BROADCAST: [
                MessageHandler(filters.ALL, handle_broadcast_state)
            ],
            MENU_STATE: [
